...
from jaxhps._precompute_operators_3D import (
    ...
    get_face_1_idxes, get_face_2_idxes, get_face_3_idxes,
    get_face_4_idxes, get_face_5_idxes, get_face_6_idxes,
)
from jaxhps.quadrature import (
    chebyshev_points, gauss_points,
...
print(f'jaxhps-ItI 2nd-order: L_PHYS={L_PHYS} p={P} q={Q} L={L} eta=-{KAPPA} '
      f'N_ITER={N_ITER}')
...
cube_faces = [
    ('xmin', 0, lambda b: np.isclose(b[0], -L_PHYS), 1, 2),  # axis=0; tang axes y(1), z(2)
...
for face_idx_check in [0, 1, 2]:
    ...
    for s in sel:
        ...
        lb = leaf_bounds[li]
        print(f'    cube_face={cf} -> volume_leaf={li} '
...
def update_bdry(u_volume):
    ...
    for cf, li, st in mapping:
        ...
        lap_face_gauss = np.asarray(per_face_lap_to_gauss(u_leaf, cf))  # (q^2, N_TX)
...
def measure_M(u_volume):
    ...
    leaf_corners = np.stack([np.array([[n.xmin, n.ymin, n.zmin],
                                        [n.xmax, n.ymax, n.zmax]])
                             for n in leaves])
    bx = ((pts_arr[:, 0, None] >= leaf_corners[None, :, 0, 0]) &
    ...
    for i in range(u_volume.shape[-1]):
        f = u_volume[patch, :, i]
        vals = vmapped_interp_to_point_3D(jnp.array(pts_arr[:, 0]),
                                          jnp.array(pts_arr[:, 1]),
                                          jnp.array(pts_arr[:, 2]),
                                          corn, f, P)
        M_cols.append(vals)
...
for it in range(N_ITER):
    ...
    if M_mie is not None:
        ...
        if err_val < best_err:
            ...
            best_M = M_now.copy()
            best_iter = it
    print(f'  {it:4d} | {float(jnp.linalg.norm(bdry)):.3e} | '
          f'{norm_M:.4e} | {err_str} | {rec:.2e}  ({sec:.0f}s)')

    new_bdry_raw = update_bdry(u_scat)
    new_bdry = DAMP * new_bdry_raw + (1.0 - DAMP) * bdry
    diff = float(jnp.linalg.norm(new_bdry - bdry) /
                 (jnp.linalg.norm(new_bdry) + 1e-30))
